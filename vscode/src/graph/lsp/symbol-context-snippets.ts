...
async function getSymbolSnippetForNodeType(
    ...
    async function getSnippetForLocationGetter(
        ...
        if (lines(definitionString).length > 50) {
            return symbolContextSnippet
        }
        ...
        const fullFirstDefinitionLine = definitionDocument.getText(
            new vscode.Range(
                definitionLocation.range.start,
                definitionLocation.range.start.translate({ lineDelta: 10 })
            )
        )
        ...
        const symbolsSnippetRequests = getLastNGraphContextIdentifiersFromString({
            ...
            .map(request => {
                if (isHover) {
                    const defLines = lines(fullFirstDefinitionLine)
